...
os_check(){	
	...
	target=(`lsblk|awk '{print$1}'|egrep -o  'sd[a-z]|nvme[0-9]{1,2}n1|pmem[0-1]' |uniq |grep -v ${os}`)
}
...
check_config () {
	...
	dmidecode -t 17 |grep "Manufacturer: Samsung\|Manufacturer: Hynix\|Manufacturer: Intel" -c >> "$log_dir"/$1_Config.txt
	echo -n "NVMe SSD Count: " >> "$log_dir"/$1_Config.txt
	nvme list |grep "/dev" -c >> "$log_dir"/$1_Config.txt
	echo "" >> "$log_dir"/$1_Config.txt
	
	fuck="`lspci -vvv |grep -i "Mellanox Technologies MT27800" |grep -v "Interrupt\|CESta" |awk '{print$1}'`"
	for i in $fuck;do
...
#	iperf -c 192.168.0.1 -i 5 -P 5 -t $1 |tee -a ""$log_dir"/iperf.log" &
    ...
    for i in "${target[@]}";do
        ...
        fio --filename=/dev/$i -direct=1 -iodepth=64 -ioengine=libaio -rw=rw -bs=128k -numjobs=4 -runtime=$1 -group_reporting -name=stress -time_based --log_avg_msec=1000 --write_iops_log="$log_dir"/"${i}"_IOpsDiag.txt >> "$log_dir"/"${i}".log &
...
`cat $c|grep -i ${err_key}`"|tee -a "$log_dir"/Summary.log
	...
	list_fio=(`ls "$log_dir"/nvme*n1.log "$log_dir"/pmem[0-1].log`)
